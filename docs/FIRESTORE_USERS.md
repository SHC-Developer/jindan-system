...
- **주의**: `이름`, `직급`, `역할` 같은 한글 필드명은 인식되지 않습니다. 반드시 `displayName`, `jobTitle`, `role` 로 넣어야 합니다.
...
4. 저장 후 **브라우저에서 새로고침** (F5) 하면 홈페이지에 반영됩니다.

---

## 반영이 안 될 때

- 필드 이름이 `displayName`, `jobTitle`, `role` 인지 확인 (대소문자 포함)
- 문서가 **컬렉션 `users`** 안에 있고, **문서 ID가 UID**인지 확인
- Firestore 저장 후 **페이지 새로고침** 후 다시 확인
